...
#' `parent_knn_across_groups` ensures that parents also come from a different
#' group.
#'
...
#' @param groups An integer vector representing the group of the each location.
#'
...
#' @describeIn parent_knn Calculate kNN parents such that parents must come
#' from a different group to the location.
#' @export
parent_knn_across_groups <- function(x, groups, n_parents, leaf_size = 40) {
  if (is.null(dim(x))) {
    x <- matrix(x, nrow = length(x))
  }
  .parent_knn_across_groups(x, groups, n_parents, leaf_size)
}
